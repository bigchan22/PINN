...
coupon = [0.091*0.5, 0.091*1.0, 0.091*1.5, 0.091*2.0, 0.091*2.5, 0.091*3.0] #잔존기별 쿠폰 2.5y 2y 1.5y 1y 0.5y 0y
...
for epoch in tqdm(range(epochs)):
    ...
    while (loss_u > 1e-5 or loss_ku > 1e-5) and _ < 500:
        ...
        ku_lb_x_x = torch.autograd.grad(inputs=lb_x, outputs=ku_lb_x,
           grad_outputs=torch.ones_like(ku_lb_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        ku_lb_x_xx = torch.autograd.grad(inputs=lb_x, outputs=ku_lb_x_x,
             grad_outputs=torch.ones_like(ku_lb_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        ku_lb_y_y = torch.autograd.grad(inputs=lb_y, outputs=ku_lb_y,
         grad_outputs=torch.ones_like(ku_lb_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ku_lb_y_yy = torch.autograd.grad(inputs=lb_y, outputs=ku_lb_y_y, 
            grad_outputs=torch.ones_like(ku_lb_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        
        ku_ub_x_x = torch.autograd.grad(inputs=ub_x, outputs=ku_ub_x, 
         grad_outputs=torch.ones_like(ku_ub_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        ku_ub_x_xx = torch.autograd.grad(inputs=ub_x, outputs=ku_ub_x_x, 
            grad_outputs=torch.ones_like(ku_ub_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        ku_ub_y_y = torch.autograd.grad(inputs=ub_y, outputs=ku_ub_y, 
            grad_outputs=torch.ones_like(ku_ub_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ku_ub_y_yy = torch.autograd.grad(inputs=ub_y, outputs=ku_ub_y_y, 
            grad_outputs=torch.ones_like(ku_ub_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
