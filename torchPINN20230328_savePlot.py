...
coupon = [0.091*0.5, 0.091*1.0, 0.091*1.5, 0.091*2.0, 0.091*2.5, 0.091*3.0] # 잔존만기별 쿠폰 2.5y 2y 1.5y 1y 0.5y 0y
...
nn5_1 = torch.autograd.grad(inputs=tensor1_to_device, outputs=nn5, grad_outputs=torch.ones_like(nn5), retain_graph=True, create_graph=True)[0]
...
nn5_xy = torch.autograd.grad(inputs=tensor1_to_device, outputs=nn5_x, grad_outputs=torch.ones_like(nn5_x), retain_graph=True, create_graph=True)[0]
nn5_yx = torch.autograd.grad(inputs=tensor1_to_device, outputs=nn5_y, grad_outputs=torch.ones_like(nn5_y), retain_graph=True, create_graph=True)[0]
...
#         du_dxx = torch.autograd.grad(inputs=self.X, outputs=du_dX, grad_outputs=torch.ones_like(du_dX), retain_graph=True, create_graph=True)[0][:, 0]
        ...
        u = NN5(txy)
        u_1 = torch.autograd.grad(inputs=txy, outputs=u, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]
        u_t = u_1[:,0].reshape(-1,1)
        u_x = u_1[:,1].reshape(-1,1)
        u_y = u_1[:,2].reshape(-1,1)
        u_2 = torch.autograd.grad(inputs=txy, outputs=u_x, grad_outputs=torch.ones_like(u_x), retain_graph=True, create_graph=True)[0]
        u_xx = u_2[:,1].reshape(-1,1)
        u_xy = u_2[:,2].reshape(-1,1)
        u_yy = torch.autograd.grad(inputs=txy, outputs=u_y, grad_outputs=torch.ones_like(u_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ...
        u_lb_x_x = torch.autograd.grad(inputs=lb_x, outputs=u_lb_x, grad_outputs=torch.ones_like(u_lb_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        u_lb_x_xx = torch.autograd.grad(inputs=lb_x, outputs=u_lb_x_x, grad_outputs=torch.ones_like(u_lb_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        u_lb_y_y = torch.autograd.grad(inputs=lb_y, outputs=u_lb_y, grad_outputs=torch.ones_like(u_lb_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        u_lb_y_yy = torch.autograd.grad(inputs=lb_y, outputs=u_lb_y_y, grad_outputs=torch.ones_like(u_lb_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        
        u_ub_x_x = torch.autograd.grad(inputs=ub_x, outputs=u_ub_x, grad_outputs=torch.ones_like(u_ub_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        u_ub_x_xx = torch.autograd.grad(inputs=ub_x, outputs=u_ub_x_x, grad_outputs=torch.ones_like(u_ub_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        u_ub_y_y = torch.autograd.grad(inputs=ub_y, outputs=u_ub_y, grad_outputs=torch.ones_like(u_ub_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        u_ub_y_yy = torch.autograd.grad(inputs=ub_y, outputs=u_ub_y_y, grad_outputs=torch.ones_like(u_ub_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        
        u_txy_ki = NN5(txy_ki)
        
        ku = NN6(txy)           
        ku_1 = torch.autograd.grad(inputs=txy, outputs=ku, grad_outputs=torch.ones_like(ku), retain_graph=True, create_graph=True)[0]
        ku_t = ku_1[:,0].reshape(-1,1)
        ku_x = ku_1[:,1].reshape(-1,1)
        ku_y = ku_1[:,2].reshape(-1,1)
        ku_2 = torch.autograd.grad(inputs=txy, outputs=ku_x, grad_outputs=torch.ones_like(ku_x), retain_graph=True, create_graph=True)[0]
        ku_xx = ku_2[:,1].reshape(-1,1)
        ku_xy = ku_2[:,2].reshape(-1,1)
        ku_yy = torch.autograd.grad(inputs=txy, outputs=ku_y, grad_outputs=torch.ones_like(ku_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ...
        ku_lb_x_x = torch.autograd.grad(inputs=lb_x, outputs=ku_lb_x, grad_outputs=torch.ones_like(ku_lb_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        ku_lb_x_xx = torch.autograd.grad(inputs=lb_x, outputs=ku_lb_x_x, grad_outputs=torch.ones_like(ku_lb_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        ku_lb_y_y = torch.autograd.grad(inputs=lb_y, outputs=ku_lb_y, grad_outputs=torch.ones_like(ku_lb_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ku_lb_y_yy = torch.autograd.grad(inputs=lb_y, outputs=ku_lb_y_y, grad_outputs=torch.ones_like(ku_lb_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        
        ku_ub_x_x = torch.autograd.grad(inputs=ub_x, outputs=ku_ub_x, grad_outputs=torch.ones_like(ku_ub_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)
        ku_ub_x_xx = torch.autograd.grad(inputs=ub_x, outputs=ku_ub_x_x, grad_outputs=torch.ones_like(ku_ub_x_x), retain_graph=True, create_graph=True)[0][:,1].reshape(-1,1)

        ku_ub_y_y = torch.autograd.grad(inputs=ub_y, outputs=ku_ub_y, grad_outputs=torch.ones_like(ku_ub_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
        ku_ub_y_yy = torch.autograd.grad(inputs=ub_y, outputs=ku_ub_y_y, grad_outputs=torch.ones_like(ku_ub_y_y), retain_graph=True, create_graph=True)[0][:,2].reshape(-1,1)
            ...
            if _%500 == 0 and epoch % 1 == 0 :
                ...
                print('loss_ku : {}\n'\
                      ...
                      'loss_ub_x_ku : {}, loss_ub_y_ku : {}'.format(loss_ku, loss_pde_ku,
                           ...
                           loss_ac_0_ku, loss_lb_x_ku, loss_lb_y_ku, loss_ub_x_ku, loss_ub_y_ku))
...
print('loss_ku : {}\n'\
      ...
      'loss_lb_x_ku : {}, loss_lb_y_ku : {}\n'\
      'loss_ub_x_ku : {}, loss_ub_y_ku : {}'.format(loss_ku, loss_pde_ku,
        loss_ic_ku, loss_ac_4_ku, loss_ac_3_ku, loss_ac_2_ku, loss_ac_1_ku, loss_ac_0_ku,
